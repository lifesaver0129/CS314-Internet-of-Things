...
//                    break;
    ...
    public void RecieveDataMsgUDP(int tranModel, DatagramPacket rcvPacket) throws Exception {
        byte[] buffer = rcvPacket.getData();
                ...
                case (byte) 0x03: //基础消息回复
                    switch (buffer[6]){  //功能码
                        ...
                        case TransFunc.PdaSensor:  //pda 传感器上传
                            ...
                            ack[8] = CRC8.calcCrc8(ack, 1, 7);
                            if (tranModel == 0){
                                DatagramPacket sndPacket = new DatagramPacket(ack, ack.length, rcvPacket.getAddress(), rcvPacket.getPort());
                                ds.send(sndPacket);
                            }
//                                out.write(ack);
                            break;
...
//                        out.write(ack);
                    ...
                    byte[] sendTime = sendControlMsg((byte) 0x21, TransFunc.GIVETIME, timeCalendar());
                    if (tranModel == 0) {
                        DatagramPacket sndPacket = new DatagramPacket(sendTime, sendTime.length, rcvPacket.getAddress(), rcvPacket.getPort());
                        ds.send(sndPacket);
                    }
//                        out.write(sendTime);
                    break;  //登陆验证
