...
/**
 * Created by fantai-xyz on 2017/9/11.
 */
public class CRC16 {
   /**
   * author fantai-xyz
   * brief 
   * params 
   * time 2017/9/26 10:07 
   * return 
   */
    public static byte[] crc16(byte[] buf,int start,int len) {
        int checkCode = 0;
        checkCode = crc_16_CCITT_False(buf, start,len);
     ...
     * @return
     */
    public static int crc_16_CCITT_False(byte[] bytes, int startPos,int length) {
        int crc = 0xffff; // initial value
        int polynomial = 0x1021; // poly value
        for (int index = startPos; index <(length + startPos); index++) {
            byte b = bytes[index];
            for (int i = 0; i < 8; i++) {
