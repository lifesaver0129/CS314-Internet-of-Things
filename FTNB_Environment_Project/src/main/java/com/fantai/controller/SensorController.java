...
public class SensorController {
    ...
    @RequestMapping("/findByDevice.do")
    public String findByDevice(Model model, HttpServletRequest request) {
        UserInfo u = (UserInfo) request.getSession().getAttribute("user");
        List<DeviceInfo> list = sensorInfoMapper.findByCodeNew("0F000007");
        model.addAttribute("module", "sensor_list");
        model.addAttribute("deviceinfos", list);
        return "sensor/findByDevice";
